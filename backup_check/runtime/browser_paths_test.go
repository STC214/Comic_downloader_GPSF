...
func TestBrowserPathsResolveExact(t *testing.T) {
	...
	match, ok = paths.ResolveExact("F:/Project/comic_downloader_GO_Playwright_stealth/runtime/browser-profiles/chromium")
	if !ok {
		t.Fatal("ResolveExact(chromium userdata) = false, want true")
	}
	if match.Kind != BrowserPathKindChromiumUserDataDir {
		t.Fatalf("match.Kind = %q, want %q", match.Kind, BrowserPathKindChromiumUserDataDir)
	}
