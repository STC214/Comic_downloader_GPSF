...
// Resolve returns the actual source profile directory for the requested browser family.
func (r BrowserProfileSourceResolver) Resolve(browserType BrowserType) (string, error) {
	switch browserType {
	case BrowserTypeFirefox:
		return r.ResolveFirefox()
	case BrowserTypeChromium:
		return r.ResolveChromium()
	default:
...
// ResolveChromium returns the actual Chromium-family user data root on the local machine.
func (r BrowserProfileSourceResolver) ResolveChromium() (string, error) {
	candidates := []string{
		filepath.Join(r.LocalAppData, "Chromium", "User Data"),
		filepath.Join(r.LocalAppData, "Google", "Chrome", "User Data"),
		filepath.Join(r.LocalAppData, "Microsoft", "Edge", "User Data"),
	}
	...
	return "", errors.New("no chromium-family user data directory found")
}
