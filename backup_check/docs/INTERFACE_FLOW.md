...
- Chromium support remains in the codebase for internal/probe use, but the public UI no longer exposes Chromium-specific controls.
- The browser session boundary uses Playwright persistent context with a copied temporary profile directory when the route needs one.
...
- Chromium probes can still use a fresh temporary Playwright profile when explicitly configured; the public UI does not expose mother-profile pickers.
