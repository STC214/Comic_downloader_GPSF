...
// ExecutionResult describes the resolved Hentai2 summary/reader flow.
...
// IsHentai2URL reports whether the URL belongs to Hentai2.
func IsHentai2URL(raw string) bool {
	...
	return host == "hentai2.net" || strings.HasSuffix(host, ".hentai2.net")
}
...
func ExecuteWithProgress(session browser.BrowserPageActions, summaryURL string, progress zeri.DownloadProgressFunc) (ExecutionResult, error) {
	...
	log.Printf("hentai2 execute start: summary=%s", summaryURL)
	...
	log.Printf("hentai2 summary parsed: title=%q pages=%d readerURL=%s", summary.Title, summary.PageCount, summary.ReaderURL)
	...
	log.Printf("hentai2 reader goto: %s", summary.ReaderURL)

	reader, err := hydrateReaderPage(session, summaryURL, summary.ReaderURL, summary.PageCount)
	if err != nil {
		return ExecutionResult{}, err
	}
...
func hydrateReaderPage(session browser.BrowserPageActions, summaryURL, pageURL string, expectedPageCount int) (ReaderPage, error) {
	initialHTML, err := session.Content()
	if err != nil {
		return ReaderPage{}, fmt.Errorf("read reader html %q: %w", pageURL, err)
	}
	page, err := ParseReaderPage(summaryURL, pageURL, initialHTML, expectedPageCount)
	if err != nil {
	...
	if expectedImages > 0 && len(page.FilteredImageURLs) >= expectedImages {
		log.Printf("hentai2 reader lazy wait skipped: url=%s resolvedImages=%d expectedImages=%d",
			pageURL, len(page.FilteredImageURLs), expectedImages)
		return page, nil
	}
	log.Printf("hentai2 reader lazy wait: url=%s expectedImages=%d", pageURL, expectedImages)
	...
	activatedPage, err := ParseReaderPage(summaryURL, pageURL, finalHTML, expectedPageCount)
	if err != nil {
		return ReaderPage{}, err
	}
	log.Printf("hentai2 reader activated parse: url=%s title=%q imageURLs=%d filtered=%d", pageURL, activatedPage.Title, len(activatedPage.ImageURLs), len(activatedPage.FilteredImageURLs))
