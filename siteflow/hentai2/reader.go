...
var (
	...
	readerBlockRe   = regexp.MustCompile(`(?is)<div[^>]*class=["'][^"']*\bread1\b[^"']*\btext-center\b[^"']*["'][^>]*>(.*?)</div>`)
...
// ReaderPage describes the important reader-page fields for Hentai2.
...
// ParseReaderPage parses a Hentai2 reader page and extracts image candidates.
func ParseReaderPage(baseURL, readerURL, pageHTML string, expectedPageCount int) (ReaderPage, error) {
	...
	log.Printf("hentai2 reader parse: url=%s title=%q imageURLs=%d filtered=%d signature=%q expected=%d",
		readerURL,
		title,
		len(imageURLs),
		len(filtered),
